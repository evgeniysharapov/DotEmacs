...
*** Search in Files
    ...
    #+begin_src emacs-lisp
      (use-package ag
        ...
        :init (progn
                ;; add ag-filter for windows
                (defun ag-windows-filter ()
                  "Handle match highlighting escape sequences inserted by the ag process.
      This function is called from `compilation-filter-hook'. This works only when `ag-highlight-search' is NIL"
                  (unless ag-highlight-search
                    (save-excursion
                      (forward-line 0)
                      (let ((end (point)) beg)
                        ...
                        (when (< (point) end)
                          ...
                          ;; Highlight match to the regex taken from
                          ;; minibuffer history
                ...
                ;; advise ag-filter function
                (add-function :after (symbol-function 'ag-filter) #'ag-windows-filter)))
    #+end_src
...
** C/C++
   ...
   #+begin_src emacs-lisp
     (setq ffy-c-headers-locations
           (append '("/usr/include/c++/4.2.1" )
                   (if (boundp 'ffy-c-headers-locations)
...
** Groovy
   #+begin_src emacs-lisp
     (use-package groovy-mode
       ...
       :config (progn
                 (fset 'stringify
                       (lambda (&optional arg) "Turns buffer into a string." (interactive "p") (kmacro-exec-ring-item (quote ([134217765 17 10 return 92 114 92 110 return 33 134217788 134217765 34 delete return 92 34 return 33 134217788 34 delete 5 34] 0 "%d")) arg)))

                 (fset 'unstringify
                       (lambda (&optional arg) "Turns string in a buffer into a code" (interactive "p") (kmacro-exec-ring-item (quote ([delete 5 backspace 134217788 134217765 92 114 92 110 return 17 10 return 33 134217788 134217765 92 34 return 34 delete return 33 134217790] 0 "%d")) arg)))
                 )
